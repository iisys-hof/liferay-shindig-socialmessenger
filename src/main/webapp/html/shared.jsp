<script type="text/javascript">
	...
	function <portlet:namespace/>createTableRow(msgId,sender,title,msg,date,recipients,collectionId,type, highlighted) {
		...
		var html = '<div class="card card-horizontal" id="<%= renderResponse.getNamespace()%>message-'+msgId+'">';
		html += '<div class="card-row';
		if(highlighted===true) html += ' highlighted';
		html += '">';
			
		html += '<div class="card-col-1 taglib-user-display display-style-2"> <a href=""> '+
			'<span class="user-profile-image"> '+
				'<img alt="" style="border-radius: 100%;" class="avatar user-img-';
					if(sender === <portlet:namespace/>LBL_YOU) html += recipients[0];
					else html += sender;
		html +=	'" src="'+<portlet:namespace/><portlet:namespace/>USER_DUMMY_PIC_URL+'">'+
			' </span> '+
			'<span class="user-name user-'+sender+'" style="display:none;"> '+sender+
			' </span> </a> <div class="user-details"> </div> </div>';
			
		html += '<div class="card-col-11 last-thread"> <div class="message-link" style="padding: 0.5em;"> '+
		...
		html += reci+' </span>'+
				'<span class="date" style="float:right; font-size: 0.8em;"> '+<portlet:namespace/>displayTime(date)+
				...
				'<div class="subject" style="font-weight:bold;"> '+title+'</div> '+
				'<div class="body">'+
